...
class CustomerDashboardApiController extends Controller
{
    ...
    public function getServices(Request $request)
    {
        try {
            ...
            $query = Product::with(['enterprise'])
                ->where('is_active', true)
                ->select('product_id', 'product_name', 'description', 'base_price', 'enterprise_id');
            ...
            if ($request->has('category') && !empty($request->category)) {
                ...
                if (isset($categoryMap[$request->category])) {
                    $query->where('product_name', 'ILIKE', "%{$categoryMap[$request->category]}%");
                }
            }
    ...
    public function getSavedServices(Request $request)
    {
        try {
            ...
            $savedServices = SavedService::with(['product.enterprise'])
                ->where('user_id', $userId)
                ->orderBy('saved_at', 'desc')
                ->get()
                ->map(function($service) {
                    return [
                        'saved_service_id' => $service->saved_service_id,
                        'product_name' => $service->product->product_name ?? 'Unknown Product',
                        'enterprise_name' => $service->product->enterprise->name ?? 'Unknown Shop',
                        'quantity' => $service->quantity,
                        'unit_price' => $service->formatted_unit_price,
                        'total_price' => $service->formatted_total_price,
                        'saved_at' => $service->saved_at->format('M d, Y'),
                        'customizations' => $service->customizationOptions->pluck('option_name')->join(', ')
                    ];
                });
