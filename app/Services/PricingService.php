...
{
    /**
     * Calculate total price for product with customizations
     *
     * @param int $productId
     * @param int $quantity
     ...
     * @return array
     */
    public function calculatePrice(int $productId, int $quantity, array $customizationIds = [], array $context = []): array
    {
        ...
        // Apply pricing rules
        $rules = PricingRule::where('product_id', $productId)
            ->active()
    ...
    {
        $cacheKey = "pricing_tiers_{$productId}";
                ...
                if ($rule->tier_structure) {
                    foreach ($rule->tier_structure as $tier) {
                        $tiers[] = [
                            ...
                            'description' => $this->formatTierDescription($tier, $product->base_price),
                        ];
                    }
                }
            }
     ...
     * @return array
     */
    public function calculateSavings(int $productId, int $quantity, array $customizationIds = []): array
    {
        $product = Product::findOrFail($productId);
        
        // Calculate price without any discounts
        $regularPrice = ($product->base_price * $quantity) + 
                       $this->calculateCustomizationCost($customizationIds, $quantity);
        
        // Calculate actual price with discounts
        $priceBreakdown = $this->calculatePrice($productId, $quantity, $customizationIds);
     ...
     * @return array
     */
    public function estimateLeadTime(int $productId, int $quantity, array $customizationIds = [], bool $isRush = false): array
    {
        $product = Product::with('enterprise')->findOrFail($productId);
