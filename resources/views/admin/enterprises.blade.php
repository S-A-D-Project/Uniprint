...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
    ...
    <div class="bg-card border border-border rounded-xl shadow-card p-6 gradient-accent text-white">
        <div class="flex justify-between items-start mb-4">
            <div>
                <p class="text-white/70 text-sm mb-1">Total Products</p>
                <h3 class="text-3xl font-bold">{{ $stats['total_products'] }}</h3>
            </div>
            <div class="bg-white/20 p-3 rounded-lg">
                    ...
                    <tr class="hover:bg-secondary/30 transition-smooth">
                        ...
                        <td class="p-4">
                            <div class="flex items-center gap-2">
                                ...
                                <button class="p-2 hover:bg-secondary rounded-md transition-colors" title="Manage Products">
...
class EnterpriseManager {
    ...
    setupEventListeners() {
        ...
        document.querySelectorAll('[title="Manage Products"]').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const row = e.target.closest('tr');
                const enterpriseName = row.querySelector('.font-semibold').textContent;
                this.manageProducts(enterpriseName);
            });
        });
    ...
    manageProducts(enterpriseName) {
        this.showNotification(`Managing products for ${enterpriseName}`, 'info');
        // In real implementation, this would navigate to products management
    }
