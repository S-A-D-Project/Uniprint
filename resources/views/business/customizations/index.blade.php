...
@section('title', 'Customizations - ' . $product->product_name)
...
<x-ui.modal id="addCustomizationModal" title="Add Customization Option" size="md" centered>
    <form id="addCustomizationForm" action="{{ route('business.customizations.store', $product->product_id) }}" method="POST">
        @csrf
        <div class="space-y-4">
            <div>
...
function editCustomization(optionId, optionType, optionName, priceModifier) {
    ...
    form.action = `{{ route('business.customizations.update', [$product->product_id, ':optionId']) }}`.replace(':optionId', optionId);
...
function deleteCustomization(optionId, optionName) {
    showConfirmModal({
        ...
        callback: async () => {
            try {
                const response = await fetch(`{{ route('business.customizations.delete', [$product->product_id, ':optionId']) }}`.replace(':optionId', optionId), {
                    method: 'DELETE',
                    headers: {
