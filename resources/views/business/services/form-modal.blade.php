            ...
            <div class="space-y-6">
                ...
                <div class="flex gap-3 pt-6">
                    <x-ui.tooltip text="Go back without saving changes">
                        <button type="button" onclick="closeAdminServiceFormModal()" 
...
async function setServicePrimaryImage(serviceId, imageId) {
    try {
        ...
        // Reload modal content instead of full page
        const modalBody = document.getElementById('adminServiceFormModalBody');
        if (modalBody) {
            openAdminServiceFormModal('{{ route('business.services.edit', $service->service_id) }}?enterprise_id={{ $enterprise->enterprise_id }}');
        }
    } catch (err) {
        if (window.UniPrintUI && typeof window.UniPrintUI.toast === 'function') {
