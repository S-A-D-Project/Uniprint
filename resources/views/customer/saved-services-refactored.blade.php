...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
    ...
    <div class="bg-white rounded-lg shadow-sm overflow-hidden customer-card" data-service-id="{{ $savedService->product_id }}">
        <!-- Selection Checkbox -->
        <div class="absolute top-3 left-3 z-10">
            <input type="checkbox" class="service-checkbox rounded text-primary focus:ring-primary" 
                   value="{{ $savedService->product_id }}" onchange="updateSelectionCount()">
        </div>

        <!-- Remove Button -->
        <div class="absolute top-3 right-3 z-10">
            <button onclick="removeSavedService('{{ $savedService->product_id }}')" 
        ...
        <div class="aspect-video bg-gradient-to-br from-primary/10 to-primary/5 flex items-center justify-center relative">
            @if(isset($savedService->image_url) && $savedService->image_url)
                <img src="{{ $savedService->image_url }}" alt="{{ $savedService->product_name }}" 
        ...
        <div class="p-4">
            <div class="flex items-start justify-between mb-2">
                <div class="flex-1">
                    <h3 class="font-semibold text-gray-900 line-clamp-1">{{ $savedService->product_name }}</h3>
            ...
            <div class="flex gap-2">
                <button onclick="viewServiceDetails('{{ $savedService->product_id }}')" 
                        class="flex-1 customer-button-secondary text-sm px-3 py-2">
                    View Details
                </button>
                <button onclick="orderService('{{ $savedService->product_id }}')" 
...
// Service actions
function removeSavedService(productId) {
        ...
        CustomerLayout.request('/saved-services/remove', {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    document.querySelector(`[data-service-id="${productId}"]`).remove();
...
function viewServiceDetails(productId) {
